const ProjectItem = ({ data }) => {
    return (
        <div className="group project-item rounded flex flex-col p-3 w-full sm:w-96 mb-3" style={{ backgroundColor: "#0E0E0E" }}>
            <div className="project-image w-full relative h-60 rounded" style={{ backgroundImage: "url(" + data.links.image + ")" }}>
                ...
                <div className="project-links flex opacity-0 absolute top-1/2 left-1/2 gap-5 transition-opacity duration-300" style={{ transform: 'translate(-50%, -50%)' }}>
                    <a className="p-2 bg-gray-200 rounded-full" href={data.links.github} target="_blank"><img className="w-10" src="https://img.icons8.com/ios-glyphs/50/000000/github.png" /></a>
                    <a className="p-2 bg-gray-200 rounded-full" href={data.links.live} target="_blank"><img className="w-10" src="https://img.icons8.com/ios-filled/50/000000/youtube-live.png" /></a>
                </div>
